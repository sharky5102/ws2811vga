...
class particles(assembly.assembly):
    class Particle:
        ...
        def step(self, dt):
            ...
            self.v = (self.v[0] * math.pow(0.2 * self.m, dt), self.v[1] * math.pow(0.2 * self.m, dt) - 0.01)

        def render(self, t):
            M = np.eye(4, dtype=np.float32)
            transforms.scale(M, .02 * self.m, .02 * self.m, 1)
    ...
    def __init__(self, spos, start):
        ...
        for i in range(0, 200):
            ...
            v = (0.5 * math.sin(a) * random.uniform(0.9, 1.0), 0.5 * math.cos(a) * random.uniform(0.9, 1.0))
